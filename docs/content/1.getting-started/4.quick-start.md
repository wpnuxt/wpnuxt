...
::callout{type="tip"}
**Want to skip the manual setup?** Run `npm create wpnuxt@latest` to get a fully configured project with Nuxt UI, dynamic pages, menus, and a WordPress Playground for local development. See [Installation](/getting-started/installation).
::
...
::callout{type="info"}
`usePosts()` blocks navigation until data is ready — this means the page won't render until posts are loaded, which is ideal for SEO (search engines see the full content). For non-critical content like sidebars, use `{ lazy: true }` to load data in the background without blocking. See [Fetching Data](/guide/fetching-data) for details.
::
...
::callout{type="info"}
The `[...slug].vue` file is a Nuxt [catch-all route](https://nuxt.com/docs/guide/directory-structure/pages#catch-all-route). It matches any URL path (e.g. `/hello-world`, `/about`, `/blog/my-post`). The `useNodeByUri()` composable sends that path to WordPress and asks "what content lives at this URI?" — WordPress returns a post, page, or any content type registered at that URL.
::
...
## Step 10: Add SEO Meta Tags

Your blog already sets the page title in Step 7. Let's add proper meta tags for search engines and social sharing using Nuxt's `useSeoMeta()`:
...
This sets the `<title>`, `<meta name="description">`, and Open Graph tags so search engines and social platforms display your content correctly.

::callout{type="tip"}
For JSON-LD structured data, sitemaps, and canonical URLs, see the full [SEO guide](/guide/seo).
::
...
- **`useSeoMeta()`** — Set meta tags for SEO and social sharing

## What's Next?

You now have a working blog! Here's where to go from here:

- [How WPNuxt Works](/getting-started/how-it-works) — Understand the architecture and composable generation pipeline
...
- [SEO](/guide/seo) — JSON-LD, sitemaps, and canonical URLs
