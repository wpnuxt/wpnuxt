...
## Reactive Filtering with Where Args

Custom queries with `where` args combine with reactive params for server-side filtering:

```graphql [extend/queries/Events.gql]
query Events($first: Int = 20, $where: RootQueryToEventConnectionWhereArgs) {
  events(first: $first, where: $where) {
    nodes {
      ...Event
    }
  }
...
const { data: events, pending } = useEvents(params)
// Auto-refetches when any filter changes
</script>
```

The `where` args are fully typed via your GraphQL schema — your editor will autocomplete available filter fields.
...
- **[Pagination](/guide/pagination)** — Add `pageInfo` to your query for cursor-based pagination
