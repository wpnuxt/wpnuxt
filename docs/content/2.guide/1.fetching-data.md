...
The `order` and `orderField` variables are available on `usePosts()`, `usePostsByCategoryName()`, and `usePostsByCategoryId()`. Possible values for `orderField`: `DATE`, `TITLE`, `MODIFIED`, `AUTHOR`, `COMMENT_COUNT`.
...
|------------|-------------|
| `usePosts()` | All posts (default limit: 10, ordered by date descending) |
...
- **[Pagination](/guide/pagination)** — Cursor-based pagination with loadMore and page navigation
