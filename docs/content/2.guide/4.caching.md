...
Copy [`wpnuxt-revalidate.php`](https://github.com/wpnuxt/wpnuxt/blob/main/docs/examples/wpnuxt-revalidate.php) to your WordPress `mu-plugins/` directory and add to `wp-config.php`:
...
On Vercel, GraphQL responses are cached at the CDN edge. To purge the CDN cache on revalidation, add a `VERCEL_TOKEN` environment variable in your Vercel project settings:

| Variable | How to get it |
|----------|---------------|
| `VERCEL_TOKEN` | Create at [vercel.com/account/tokens](https://vercel.com/account/tokens) |

`VERCEL` and `VERCEL_PROJECT_ID` are provided automatically by Vercel at runtime. When `VERCEL_TOKEN` is set, the revalidation endpoint purges both Nitro's internal cache and Vercel's CDN cache.
