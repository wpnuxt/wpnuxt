...
```
Request 1 (t=0s):     WordPress → Cache (500ms)
Request 2 (t=60s):    Cache → User (2ms) ✓ Fresh
Request 3 (t=300s):   Cache → User (2ms) + Background Refresh ✓ Stale but instant
Request 4 (t=301s):   Cache → User (2ms) ✓ Fresh again
```
...
After Expiry:      █ 2ms (stale + background refresh)
...
```bash
[wpnuxt] ⚙ Server-side caching enabled for GraphQL requests (maxAge: 300s, SWR: true)
```
...
## Debugging with Nuxt DevTools

WPNuxt inherits DevTools integration from `nuxt-graphql-middleware`. The Nuxt DevTools provides a built-in GraphQL tab that helps you debug queries.

### Features

- **Query List**: View all GraphQL operations (queries and mutations)
- **Request/Response**: Inspect the full request and response data
- **Variables**: See the variables passed to each operation
- **Timing**: View how long each GraphQL request takes

### Enabling DevTools

DevTools are automatically enabled in development mode. Open them with:

- **macOS**: `⇧⌥D` (Shift+Option+D)
- **Windows/Linux**: `Shift+Alt+D`

Or click the Nuxt DevTools floating button in your app.

### Tips

1. **Filter by operation**: Use the search in the DevTools GraphQL tab to find specific queries
2. **Watch network**: The Network tab shows the actual HTTP requests to `/api/graphql_middleware/*`
3. **Check cache**: Compare response times - cached responses are near-instant
...
- ✅ Use SWR: `swr: true`
- ✅ Choose appropriate `maxAge` for your content
- ✅ Monitor performance with `debug: true`
- ✅ Use Nuxt DevTools to debug queries
